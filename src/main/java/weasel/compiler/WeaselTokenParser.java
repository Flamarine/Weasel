	...
	public WeaselToken getNextToken() throws WeaselSyntaxError{
		...
		try{
			...
			if(isAlphabetical(c)){
				...
				int modifier = WeaselModifierMap.getModifier(ident.toString());
				if(modifier!=0){
					return new WeaselToken(WeaselTokenType.MODIFIER, line, modifier);
				}
				WeaselKeyWord wkw = WeaselKeyWord.getWeaselKeyWord(ident.toString());
				if(wkw!=null){
					return new WeaselToken(WeaselTokenType.KEYWORD, line, wkw);
				}
				return new WeaselToken(WeaselTokenType.IDENT, line, ident.toString());
			}
			if(isDigit(c) || c=='.'){
				isEOFOk = false;
				boolean isDot = c=='.';
			...
			if(c=='"'){
				...
				return new WeaselToken(WeaselTokenType.STRING, line, s.toString());
			}
