...
public class WeaselClassCompiler extends WeaselClass {
	...
	private final WeaselTokenParser tokenParser;
	private final List<WeaselToken> nextTokens = new ArrayList<>();
	private final List<WeaselCompilerException> error = new ArrayList<>();
	...
	public WeaselToken expectToken(WeaselTokenType ...tt) throws WeaselSyntaxError{
		WeaselToken token = nextToken();
		for (WeaselTokenType weaselTokenType : tt) {
			if (token.tokenType == weaselTokenType)
				return token;
		}
		StringBuilder allowed = new StringBuilder();
		if(tt.length==1){
			allowed = new StringBuilder(tt[0].symbol);
		}else if(tt.length>1){
			for(int i=0; i<tt.length-2; i++){
				allowed.append(tt[i].symbol).append(", ");
			}
			allowed.append(tt[tt.length - 2].symbol).append(" & ");
			allowed.append(tt[tt.length - 1].symbol);
		}
		throw new WeaselSyntaxError(token.line, "Illegal token %s, only %s are permitted", token, allowed.toString());
	}
	...
	public WeaselToken expectToken(WeaselTokenType tt, Object...datas) throws WeaselSyntaxError{
		...
		if(token.tokenType==tt){
			for (Object data : datas) {
				if (token.param.equals(data))
					return token;
			}
		}
		StringBuilder allowed = new StringBuilder();
		if(datas.length==1){
			allowed = new StringBuilder(datas[0].toString());
		}else if(datas.length>1){
			for(int i=0; i<datas.length-2; i++){
				allowed.append(datas[i].toString()).append(", ");
			}
			allowed.append(datas[datas.length - 2].toString()).append(" & ");
			allowed.append(datas[datas.length - 1].toString());
		}
		throw new WeaselSyntaxError(token.line, "Illegal %s, only %s are permitted", token, allowed.toString());
	}
