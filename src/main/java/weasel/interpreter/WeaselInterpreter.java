...
public class WeaselInterpreter implements WeaselSaveable, WeaselClassBuffer {
	...
	private final WeaselMemory memory;
	private final TreeMap<String, WeaselNativeMethod> nativeMethods = new TreeMap<>();
	private final List<WeaselNativeGenerator> nativeGenerators = new ArrayList<>();
	private final TreeMap<String, WeaselClass> loadedClasses = new TreeMap<>();
	private final List<WeaselThread> threads = new ArrayList<>();
	...
	public WeaselMethod getMethodByDesk(String methodDesk) {
		...
		WeaselClass wClass = getClassByName(className);
		return wClass.getMethod(name, desk);
	}
